<template>
	<div class="add-password">
		...
		<div class="container">
			<form class="add-password__form" @submit="onSubmitCreatePassword">
				<div class="add-password__top">
					<div class="add-password__wrapper">
						<span class="add-password__label">Название группы</span>
						<div class="add-password__select">
							<div>
								<input
									class="add-password__input add-password__input--select"
						...
						<span class="add-password__error add-password__error--directory" v-if="directory_error">Выберите группу</span>
					</div>
					<div class="add-password__wrapper">
						<span class="add-password__label add-password__label--website">Название сайта</span>
						<input
							v-model="secret.website"
							type="text"
							class="add-password__input add-password__input--website"
							placeholder="Введите название сайта"
							@focus="onFocus('website')"
						>
						<span class="add-password__error add-password__error--website" v-if="website_error">Введите название сайта</span>
					</div>
					<div class="add-password__wrapper">
						<span class="add-password__label">Логин</span>
						<input
							v-model="secret.login"
							type="text"
							class="add-password__input add-password__input--login"
							placeholder="Введите логин"
							@focus="onFocus('login')"
						>
						<span class="add-password__error add-password__error--login"
							  v-if="login_error">Введите логин</span>
					</div>
					<div class="add-password__wrapper">
						<span class="add-password__label">Пароль</span>
						<input
							v-model="password_visible"
							:type="isPasswordShown?'text' : 'password'"
							class="add-password__input add-password__input-password"
							placeholder="Введите пароль"
							@focus="onFocus('password')"
							@input="onPasswordInput"
						>
						<div class="add-password__eyes" @click="onShowPassword()">
							<svg v-if="!isPasswordShown" class="add-password__eye add-password__eye--show">
								<use xlink:href="../../public/image/sprite.svg#eye-show"></use>
							</svg>
							<svg v-else class="add-password__eye add-password__eye--hidden">
								<use xlink:href="../../public/image/sprite.svg#eye-hide"></use>
							</svg>
						</div>
						<span
							class="add-password__error add-password__error--password"
							v-if="password_error"
						>Введите или сгенерируйте пароль
						</span>
					</div>
				</div>
				<div class="add-password__bottom">
					<button
						class="add-password__button add-password__button--generate"
						@click="onGeneratePassword()"
						type="button"
					>Сгенерировать новый пароль
					</button>
					<button class="add-password__button" type="submit"
							:class="validate? 'add-password__button--hover' : ''">Добавить
					</button>
				</div>
			</form>
		</div>
...
export default {
	...
	methods: {
		...mapActions(['addSecret', 'loadDirectoriesSecrets', 'generatePassword', 'decryptPassword', 'encryptPassword',
			'notify', 'addDirectory']),
		onFocus(type) {
			switch (type) {
				case 'directory':
		...
		async onSubmitCreatePassword(e) {
			e.preventDefault()
			...
			let error = await this.addSecret(this.secret)
			if (!!error) {
				console.log('error:', error)
				this.notify('Пароль не создан!')
				return
			}
			this.notify('Пароль добавлен!')
